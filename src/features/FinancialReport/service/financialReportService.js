...
import { normalizeFinancialReportRow } from '../utils/financialReportUtils'
...
const ensureFinancialPayload = (payload, fallbackMessage) => {
...
export const getFinancialReport = async ({ page = 1, search = '' } = {}) => {
  const query = search ? `&search=${encodeURIComponent(search)}` : ''
  const payload = ensureFinancialPayload(
    await apiRequest(`${API_URLS.reports.financialReport}?page=${page}${query}`),
  ...
  return {
    pagination,
    rows: rows.map((item, index) => normalizeFinancialReportRow(item, index, pagination)),
  }
}

export const emptyFinancialReportCollection = {
